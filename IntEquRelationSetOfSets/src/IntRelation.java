...
public abstract class IntRelation {
     ...
     * @throws IllegalArgumentException {@code if n < 0}
     * @pre {@code 0 <= n}
    ...
    /**
     * Checks whether a pair is valid for this relation.
     *
     * @param a  first element of the pair
     * @param b  second element of the pair
     ...
     * @pre {@code true}
     * @modifies None
     * @post {@code isValidPair(a, b)}
     */
    public void checkValidPair(int a, int b, String name)
            throws IllegalArgumentException {
        if (! isValidPair(a, b)) {
            throw new IllegalArgumentException(
                    this.getClass().getName() + "." + name + ".pre violated:"
                    + " (" + a + ", " + b + ") not a valid pair");
        }
    }
